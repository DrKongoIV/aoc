...
fn count_paths(graph : &HashMap::<String, Vec<String>>, mut visited : HashSet<String>, v : String) -> u32 {
    if v == "end" {
        return 1;
    }
    ...
    let mut sum = 0;
    for w in &graph[&v] {
        sum += count_paths(graph, visited.clone(), w.to_string());
    }
    return sum;
}
...
fn main() {
    ...
    let kek = count_paths(&graph, visited, "start".to_string());
    println!("{}", kek);
}
